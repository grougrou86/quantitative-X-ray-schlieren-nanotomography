AUTO_offset=true;%false;
default_offset_pixel=0;
num_itter_field_recon = 7;% 7;%7;
save_out=true;false;%true;

remove_ring=true;false;true;%false;

figShow=false;
set_path_pepsi();
for refocus_curve=1.5;%-2:0.5:2 %aply a frequency filtering similar to refocus to minimize border artifact arizing from slightly out of focus experiments
    %% input
    % jsonPath =[];
    ...
    %MAIN_PATH='G:\20240911_BMOL\mixture';name='\NO_FILTER_8374.000';remove_ring=true;percent_cropp=0.2;%0.2
    %MAIN_PATH='G:\20240911_BMOL\silicon';name='\NO_FILTER_8374.000';percent_cropp=0.2;shift_z=70;
    ...
    %MAIN_PATH='F:\simulation_xray_kk\sim_similar_noise_10000_focus_0_cutoff_shift_0_scint_scale_1';name='\SIM_8333.000';percent_cropp=1;norotation=true;refocus_curve=0;save_out=true;
    %MAIN_PATH='F:\simulation_xray_kk\sim_similar_noise_10000_focus_0_cutoff_shift_0_scint_scale_2';name='\SIM_8333.000';percent_cropp=1;norotation=true;refocus_curve=0;save_out=true;
    %MAIN_PATH='F:\simulation_xray_kk\sim_similar_noise_10000_focus_0_cutoff_shift_1_scint_scale_1';name='\SIM_8333.000';percent_cropp=1;norotation=true;refocus_curve=0;save_out=true;
    %MAIN_PATH='F:\simulation_xray_kk\sim_similar_noise_10000_focus_20_cutoff_shift_0_scint_scale_1';name='\SIM_8333.000';percent_cropp=1;norotation=true;refocus_curve=0;save_out=true;
    %MAIN_PATH='F:\simulation_xray_kk\sim_similar_noise_10000_focus_0_cutoff_shift_0_scint_scale_1_2';name='\SIM_8333.000';percent_cropp=1;norotation=true;refocus_curve=0;save_out=true;
    ...
    if norotation
        tH.setup.detector.camera.tiltAngle=0;
    end
    disp(jsonPathOut)
    ...
    [fourier_position,fourier_intensity,dark_position]=cut_off_analysis_general_v2(cutoff_file,val.start,val.step,figShow);
    % {
    selection=25:70;
    ...
    rotAnglelib = linspace(val.stop_angle-360, val.stop_angle, length(tH.samFiles)+1);
    rotAnglelib = rotAnglelib(1:end-1);
    ...
    [rawSino,raw_BG] = tH.getSinogram(samInds,'AUTO_offset',AUTO_offset,'figShow',figShow);
...
for pp=1:size(rawSino,1)
    ...
    for offset=possible_val;
        ...
        rawSino_rec=circshift(rawSino_rec,[shift_z 0]);
        rawSino_rec=...
            rawSino_rec(...
        ...
        [tomoOut, Eout, DPCdata] = sino2tomo(gpuArray(rawSino_rec), angleLib(validAngInd), 0, bgWingSize, fieldReconInputParams, false,rawSino_rec(:,:,1)*0+1,0);
    ...
    raw_BG_rec=circshift(raw_BG_rec,[shift_z 0]);
    raw_BG_rec=...
        raw_BG_rec(...
    ...
    if remove_ring
        ...
        tst=s_fftn(tst);
        tst([1:round(end*hig_freq) (end-round(end*hig_freq)):end],:,floor(end/2+1)+(-low_freq:low_freq))=0;
        tst(:,[1:round(end*hig_freq) (end-round(end*hig_freq)):end],floor(end/2+1)+(-low_freq:low_freq))=0;
        tst=s_ifftn(tst);
        
        rawSino_rec(:,:,:)=real(tst);
    end
    %%
    if AUTO_offset
        rawSino_rec_out=fine_register_fields_TV_v5(rawSino_rec,figShow);
    else
    ...
    fieldReconInputParams = {num_itter_field_recon, p, 'figShow',true,'DPCdata',struct([])};
    ...
    opt_fun=@(x) try_RI_recon(x,rawSino_rec_out_final,yy,xx,Nang0,angleLib,bgWingSize,fieldReconInputParams,refocus_curve,figShow);
    a=0;
    b=1;%1;
    itt_gss=8;%5;
    if~AUTO_offset
        itt_gss=0;
        b=default_offset_pixel;a=b;
    end
    invphi = (sqrt(5) - 1) / 2 ;
    comp_c=true;
    comp_d=true;
    ...
    %figure, orthosliceViewer(imag(imresize3(imresize3(RI_optimal,0.5),2))); axis image; caxis([-1 1]*1e-6); title(['Refocus of : ' num2str(refocus_curve)]);
    
    if(save_out)
        %save([MAIN_PATH '\recon_result_' num2str(num_itter_field_recon) 'itt.mat'],'RI');
        save([MAIN_PATH '\recon_result_' num2str(num_itter_field_recon) 'itt_' num2str(remove_ring) '_RingRemove.mat'],'RI');
    end
...
function [err_coo,RI]=try_RI_recon(shift_axis,rawSino_rec_out_final,yy,xx,Nang0,angleLib,bgWingSize,fieldReconInputParams,refocus_curve,figShow)
...
for ii = 1:Npart
    ...
    RI(thisChunk,:,:) = sino2tomo(gpuArray(rawSino_rec_out_final(thisChunk,:,validAngInd)), angleLib(validAngInd), shift_axis, bgWingSize, fieldReconInputParams,figShow,1,refocus_curve);
