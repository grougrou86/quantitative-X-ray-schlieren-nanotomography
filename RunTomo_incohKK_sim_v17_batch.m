...
remove_ring=false;true;%false;

figShow=false;
%set_path_pepsi();

MAIN_PATH_list={'G:\20240911_BMOL\mixture_publication'};name='\NO_FILTER_8374.000';remove_ring=true;percent_cropp=0.2;%0.2

for list_num=1:length(MAIN_PATH_list)
    for refocus_curve=1.5;%-2:0.5:2 %aply a frequency filtering similar to refocus to minimize border artifact arizing from slightly out of focus experiments
        %% input
        % jsonPath =[];
        topPath = 'G:\20240911_BMOL\code_publication'; 
        ...
        MAIN_PATH=MAIN_PATH_list{list_num};
...
for pp=1:size(rawSino,1)
    ...
    rawSino(pp,:,:)=removeStripes('h',gpuArray(squeeze(rawSino_0(pp,:,:))),2,'db45',3);
        ...
        for offset=possible_val;
            ...
            [tomoOut, Eout, DPCdata] = sino2tomo(gpuArray(rawSino_rec), angleLib(validAngInd), 0, bgWingSize, fieldReconInputParams, false,rawSino_rec(:,:,1)*0+1,0,false);
        ...
        %rawSino_rec=xRemoveStripesVertical(rawSino_rec,5,"db25",2.4);
        %rawSino_rec=xRemoveStripesVertical(rawSino_rec,5,"db25",10);
        %{
        tst=(rawSino_rec(:,:,:));
        hig_freq=0.4;
        low_freq=4;%2;
        ...
        %%
        if AUTO_offset
            rawSino_rec_out=fine_register_fields_TV_v7(rawSino_rec,figShow);
        else
        ...
        opt_fun=@(x,remove_ring) try_RI_recon(x,rawSino_rec_out_final,yy,xx,Nang0,angleLib,bgWingSize,fieldReconInputParams,refocus_curve,remove_ring,figShow);
        a=0;
        b=2;%1;%1;
        itt_gss=8;%5;
        if~AUTO_offset
        ...
        [optimal_cost,RI_optimal]=opt_fun(optimal_shift,remove_ring);
        RI=RI_optimal;
        if figShow
...
function [err_coo,RI]=try_RI_recon(shift_axis,rawSino_rec_out_final,yy,xx,Nang0,angleLib,bgWingSize,fieldReconInputParams,refocus_curve,remove_ring,figShow)

RI = zeros(yy,xx,xx,'single');
bytesPerSlice = (xx*Nang0) * 8; % 4 bytes for single
gpu=gpuDevice;
maxChunkSize = 300;%floor( gpu.AvailableMemory / bytesPerSlice / 30 ); % last factor is a heuristic safety value.
...
for ii = 1:Npart
    ...
    %rawSino_rec=xRemoveStripesVertical(rawSino_rec,5,"db25",10);
    RI(thisChunk,:,:) = sino2tomo(gpuArray(rawSino_rec_out_final(thisChunk,:,validAngInd)), angleLib(validAngInd), shift_axis, bgWingSize, fieldReconInputParams,figShow,1,refocus_curve,remove_ring);
