...
%% Node in the function evalution graph
    ...
    properties
        ...
        grad % gradient accumulator  df/dconj(z)
        grad_c % df/dz
    ...
    methods
        ...
        function [dy,dy_c] = backprop(x, dy,dy_c)
        %% backpropagate the gradient by evaluating the tape backwards
            ...
            dy_c = x.root.grad_c;
            if ~isempty(dy_c)
                if size(dy_c) ~= size(x.root.value)
                    if size(dy_c) == [1, 1]
                        dy_c = repmat(dy_c, size(x.root.value));
                    elseif size(dy_c, 1) == 1 && size(x.root.value, 1) ~= 1
                        dy_c = repmat(dy_c, size(x.root.value, 1), 1);
                    elseif size(dy_c, 2) == 1 && size(x.root.value, 2) ~= 1
                        dy_c = repmat(dy_c, 1, size(x.root.value, 2));
                    end
                end
            end
        end
        ...
        function y = sn_fft2(x)
            y = ADNode(sn_fft2(x.value), x.root, @(y) x.add(bsxfun(@times, y.grad, sn_ifft2(x.value))));
        end
        function y = sn_ifft2(x)
            y = ADNode(sn_ifft2(x.value), x.root, @(y) x.add(bsxfun(@times, y.grad, sn_fft2(x.value))));
        end
