    ...
    methods
        ...
        function [gradient] = backprop(x, dy,dy_c)
            ...
            for k = length(x.root.tape):-1:1
                ...
                %display(x.root.tape{k}.debug_var);
                %}
            ...
            if ~isempty(gradient)
                ...
                for ii=1:length(size(x.root.value))
        ...
        function y = reshape(x,sizing)
            sz=size(x.value);
            y = ADNode(reshape(x.value,sizing), x.root, @(y) x.add((reshape(y.grad,sz)),(reshape(y.grad_c,sz))));
            if ~x.reuse
                x.value=[];%save memory
            end
        end
        ...
        function y = mean(x, dim, flag)
            ...
            switch nargin
                case 3
                    y = ADNode(mean(x.value, dim, flag), x.root, @(y) x.add(y.grad./avg_size,y.grad_c./avg_size));
                    if ~x.reuse
                        x.value=[];
                    end
                case 2
                    y = ADNode(mean(x.value, dim), x.root, @(y) x.add(y.grad./avg_size,y.grad_c./avg_size));
                    if ~x.reuse
                        x.value=[];
                    end
                otherwise
                    y = ADNode(mean(x.value), x.root, @(y) x.add(y.grad./avg_size,y.grad_c./avg_size));
                    if ~x.reuse
                        x.value=[];
                    end
            end
        ...
        function y = real(x)
            y = ADNode(real(x.value), x.root,@(y) x.add(...
                set_real_flag(0.5.*(y.grad  +conj(y.grad_c)),x.root.real_optimization),...
                set_real_flag(0.5.*(y.grad_c+conj(y.grad)  ),x.root.real_optimization) ));
            y.real_valued=true;
            if ~x.reuse
                x.value=[];%save memory
            end
        ...
        function y = imag(x)
            y = ADNode(imag(x.value), x.root,@(y) x.add(...
                -0.5i.*(y.grad  +conj(y.grad_c)),...
                -0.5i.*(y.grad_c+conj(y.grad)  )));
            if ~x.reuse
                x.value=[];%save memory
            end
        end
