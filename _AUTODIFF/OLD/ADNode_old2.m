    ...
    methods
        ...
        %% backpropagate the gradient by evaluating the tape backwards
            ...
            for ii=1:length(size(x.root.value))%simplified by HH
                if size(x.root.value, ii)~=size(dy, ii) && size(dy, ii)==1
                    repetitions(ii)=size(x.root.value, ii);
                end
            end
            dy = repmat(dy,repetitions(ii));
            for ii=1:length(size(dy)) %added by HH
                if size(x.root.value, ii)~=size(dy, ii) && size(x.root.value, ii)==1
                    dy=sum(dy,ii);
                end
            end
        ...
        function y = sn_fft2(x)
            y = ADNode(sn_fft2(x.value), x.root, @(y) x.add(sn_ifft2(y.grad)));
        end
        function y = sn_ifft2(x)
            y = ADNode(sn_ifft2(x.value), x.root, @(y) x.add(sn_fft2(y.grad)));
        end
...
% horzcat
    ...
    methods (Access = private)
        function add(x, grad)
            ...
            if isempty(x.grad)
                x.grad=gradient_typing(zeros(size(x.value),'single'));
            end
            temp=grad;
