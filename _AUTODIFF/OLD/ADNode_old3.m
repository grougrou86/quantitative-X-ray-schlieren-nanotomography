...
classdef ADNode < handle
    ...
    methods
        ...
        function y = abs(x)
            ...
            y = ADNode(abs(x.value), x.root,@(y) x.add(...
                bsxfun2(@plus,bsxfun2(@times, (y.grad)  , 0.5.*sign(conj(x.value))),bsxfun2(@times, (y.grad_c),0.5.*sign(conj(x.value)))),...
                bsxfun2(@plus,bsxfun2(@times,  (conj(y.grad_c)), 0.5.*sign(x.value)),bsxfun2(@times, conj(y.grad)  ,0.5.*sign((x.value)))) ));
        end
        ...
        function y = log(x)
            y = ADNode(log(x.value), x.root, @(y) x.add(bsxfun2(@rdivide, y.grad, x.value),bsxfun2(@rdivide, y.grad_c, x.value)));
        end

        function y = sin(x)
            y = ADNode(sin(x.value), x.root, @(y) x.add(bsxfun2(@times, y.grad, cos(x.value)),bsxfun2(@times, y.grad_c, cos(x.value))));
        end

        function y = tan(x)
            y = ADNode(tan(x.value), x.root, @(y) x.add(bsxfun2(@times, y.grad, sec(x.value) .^ 2),bsxfun2(@times, y.grad_c, sec(x.value) .^ 2)));
        end
    ...
    methods (Access = private)
        ...
        function atan2_backprop(y,x1,x2)
            x.add_times_holo(y, x2.value ./ (x2.value.^2 + x1.value.^2));
            x.add_times_holo(y, -x1.value ./ (x2.value.^2 + x1.value.^2));
        end
        ...
        function times_backprop(y, x1, x2)
            x1.add(bsxfun2(@times, y.grad, (x2.value)),bsxfun2(@times, y.grad_c, (x2.value)));
            x2.add(bsxfun2(@times, y.grad, (x1.value)),bsxfun2(@times, y.grad_c, (x1.value)));
        end
