function [phi_uw_iter,U_iter,V_iter,residue0] = unwrap2_Lp(phi_w, varargin)
...
%% inputParser
pa = inputParser();
addOptional(pa, 'p',  0, @isscalar);
addParameter(pa, 'tol',  1e-3, @isscalar);
addParameter(pa, 'iterMax',  1000, @isscalar);
addParameter(pa, 'epsilon0',  0.01, @isscalar);
addParameter(pa, 'weight0', []);
...
p = pa.Results.p;
tol = pa.Results.tol;
iterMax = pa.Results.iterMax;
epsilon0 = pa.Results.epsilon0;
weight0 = pa.Results.weight0;


%% ALgorithm Lp-norm (Section 3.B)
%%% step 1
assert(p <= 2,'Select p < 2.')
...
while true
    ...
    if (~residueN) || p == 2 % if no residue exsits
        ...
        if nargout > 2
            [U_iter,V_iter] = Wfunc(phi_uw_iter,f0,g0,p,epsilon0,weight0); % Eq. (41-42)
        end
...
function [U,V] = Wfunc(phi,f,g,p,epsilon0,weight0)
